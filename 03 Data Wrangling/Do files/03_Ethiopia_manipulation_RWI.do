*************************************************************************
*	Purpose: Create dummy variables for Ethiopia PPI - RWI data			*
...
global clean "$path/02 Data/02 Clean/RWI"
...
*Personal global (may change the path)
	*No need to change
	...
	global rwi "$path/02 Data/02 Clean/RWI"
...
******************************************
******************************************
import delimited "$path/02 Data/01 Raw/GPS/eth_cluster_RWI.csv", clear
...
******************************************
******************************************
merge 1:m cluster_lat cluster_lon using "$clean/$odb"
keep if _merge==3
drop _merge
...
saveold "$rwi/$ppidb", replace version(12)
				...
				*Unique_var_name has some jumps (e.g. from 60 to 69). We don't want this to happen. So we wil fix it (e.g. so after 60 goes 61).
				...
				export delimited using "$rwi/$qkey",  replace
