...
bootstrap_variable_select_ologit <-function(data_boot, bootstrap_sample_fraction,
           bootstrap_reps, question_key, y, numb_questions, alpha, weight,
           penalty_vector= penalty_vector_full, fam = "binomial", iter=0) {
    ...
    for (i in 1:bootstrap_reps) {
      ...
      coeff_matrix[i, ] <- coefficients_elastic_net_ologit(data_var_select = data_boot[sample_rows, ], 
