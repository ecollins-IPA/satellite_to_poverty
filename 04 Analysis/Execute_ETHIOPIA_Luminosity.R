...
question_key <-"Ethiopia15-16_QuestionKEY.csv" #Key file name
...
candidate_feature_last_col <- 61 #the column number of the last candidate indicator
...
for (nq in numb_questions) { 
  ...
  for (pov in target) {
    ...
    targetting_error <- rbind(targetting_error, error_rural,error_urban,error_full) #Put together all 
    ...
    poverty_error <- rbind(poverty_error, pov_error_ci_rural, pov_error_ci_urban, pov_error_ci_full) #Put together all error rates
