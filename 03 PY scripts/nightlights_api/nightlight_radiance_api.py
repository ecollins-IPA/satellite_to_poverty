...
def get_max_radiance(country, tif):
    ...
    with open('max-radiance-per-country.json', 'w') as f:
...
def get_normalized_mean_radiance(
    country, coordinates_list, grid_length):
    """
    ...
    Input:
        ...
        (lat, lon) list: Coordinates of point whose nightlight intensity we want to compute
    Returns list float
    """
    ...
    for (lat, lon) in coordinates_list:
        ...
        normalized_mean_radiance = np.log(mean_radiance+1)/np.log(max_radiance)

        #Add to list
        normalized_mean_radiances.append(normalized_mean_radiance)

        print(f'Computed radiance for {(lat,lon)}: {normalized_mean_radiance}')

    return normalized_mean_radiances
...
if __name__=='__main__':
    ...
    grid_length = 10
    coordinates_list = [(eth_random_point_lat, eth_random_point_lon)]
